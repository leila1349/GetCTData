...
*/
  ...
  nz = files.size();
  ny = ct.getNumberOfRows();
  nx = ct.getNumberOfColumns();

  cout << "Located number of reconstructed slices in tif format: " << nz << endl;
  ...
  // find out from John what to do wit these tags
   ...
   tbuf =_TIFFmalloc(TIFFScanlineSize(tFile));  
    ...
    check_io = TIFFReadScanline(tFile, tbuf, column, 0);
    if (check_io != 1)
     {
